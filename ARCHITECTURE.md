...
`public/data` and `public/maps` are regenerated by a build step and never edited
in place, so `vercel.json` marks them `immutable` for a year. Note that Vercel
matches `source` with path-to-regexp, not a regular expression: an alternation
group like `/(data|maps)/(.*)` matches nothing and fails silently, leaving the
default `s-maxage=0`. Two `:path*` rules are needed instead. Caught by checking
the response headers on the deployed URL rather than trusting the config.
